...
timeout 에러 발생. 해결 방법은 간단했다.

for문에서 나무를 자르고 얻을 수 있는 길이를 sum에 더하는데 

for문 수행 도중에 sum이 M보다 길이가 길면 break 하도록 수정. (for문을 다 수행할 필요가 없다.)
...
테스트는 통과했는데 시간이 오래걸려 확인해보니 list.sort()의 `시간 복잡도가 O(NlogN)`이였다.

max()와 min()의 `시간복잡도는 O(N)`. sort() 보단 max()가 더 나은듯..